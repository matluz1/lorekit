...
def test_create_pc(run, make_session):
    sid = make_session()
    r = run("character.py", "create", "--session", sid, "--name", "Thalric", "--type", "pc", "--level", "1")
...
def test_view_character(run, make_session, make_character):
    sid = make_session()
    cid = make_character(sid, "Elara", "pc")
    r = run("character.py", "view", cid)
    assert "NAME: Elara" in r.stdout
...
def test_set_and_get_ability(run, make_session, make_character):
    ...
    run("character.py", "set-ability", cid, "--name", "Flame Burst", "--desc", "3d6 fire damage", "--category", "spell", "--uses", "3/day")
    r = run("character.py", "get-abilities", cid)
    assert "Flame Burst" in r.stdout
