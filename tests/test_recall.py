...
def test_reindex_timeline(make_session):
    sid = make_session()
    ...
    assert "2 timeline entries, 0 journal entries" in result
...
def test_reindex_both(make_session):
    sid = make_session()
    timeline_add(session_id=sid, type="narration", content="Entered the city gates")
...
def test_search_finds_relevant(make_session):
    sid = make_session()
    timeline_add(session_id=sid, type="narration", content="The party entered the ancient temple")
    timeline_add(session_id=sid, type="narration", content="A merchant sold them potions")
...
def test_search_source_timeline(make_session):
    sid = make_session()
    timeline_add(session_id=sid, type="narration", content="Discovered the lost shrine")
...
def test_search_n_controls_results(make_session):
    ...
    sid = make_session()
    for i in range(5):
        timeline_add(session_id=sid, type="narration", content=f"Adventure event number {i}")
    recall_reindex(session_id=sid)
...
def test_reindex_rebuilds_all_sessions(make_session):
    ...
    sid1 = make_session()
    sid2 = make_session()
    timeline_add(session_id=sid1, type="narration", content="First session event")
    timeline_add(session_id=sid2, type="narration", content="Second session event")
