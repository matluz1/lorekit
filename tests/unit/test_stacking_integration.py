...
class TestCombatStateLoading:
    ...
    def test_combat_modifier_affects_derived(self, make_session, make_character):
        ...
        from lorekit.rules import load_character_data, load_combat_modifiers
        ...
        mods = load_combat_modifiers(db, cid)
        result_before = recalculate(pack, char, modifiers=mods)
        ...
        mods = load_combat_modifiers(db, cid)
        result_after = recalculate(pack, char, modifiers=mods)
...
class TestTypedStackingIntegration:
    ...
    def test_same_type_doesnt_stack(self, make_session, make_character):
        ...
        char = load_character_data(db, cid)
        mods = load_combat_modifiers(db, cid)
        result_base = recalculate(pack, char, modifiers=mods)
        ...
        mods = load_combat_modifiers(db, cid)
        result = recalculate(pack, char, modifiers=mods)
...
class TestBuildEngineTypedEffects:
    ...
    def test_dict_format_effects(self):
        from cruncher.build import BuildResult, _apply_effects
