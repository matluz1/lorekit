...
test_create_pc() {
    ...
    local output
    output=$(bash "$CHAR" create --session "$sid" --name "Thalric" --type pc)
...
test_view_character() {
    local sid cid
    sid=$(create_test_session)
    cid=$(create_test_character "$sid" "Elara" "pc")
    local output
    output=$(bash "$CHAR" view "$cid")
    assert_contains "$output" "NAME: Elara"
...
test_set_and_get_ability() {
    ...
    bash "$CHAR" set-ability "$cid" --name "Flame Burst" --desc "3d6 fire damage" --category spell --uses "3/day" > /dev/null
    local output
    output=$(bash "$CHAR" get-abilities "$cid")
    assert_contains "$output" "Flame Burst"
