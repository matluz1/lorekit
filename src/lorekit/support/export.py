...
def clean(db) -> str:
    if os.path.isdir(_get_export_dir()):
        shutil.rmtree(_get_export_dir())
        return f"CLEANED: {_get_export_dir()}"
    else:
...
def dump(db, session_id: int) -> str:
    ...
    os.makedirs(_get_export_dir(), exist_ok=True)
    output_file = os.path.join(_get_export_dir(), f"session_{session_id}.txt")
